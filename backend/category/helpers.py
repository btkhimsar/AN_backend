...
def request_json_for_category(category, language):
    request_data = {'category_id': str(category.id), 'name': category.name[language],
...
def request_json(super_category, fetched_categories_dict, language):
    request_data = {'super_category_id': str(super_category.id), 'name': super_category.name[language],
