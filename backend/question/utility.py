def question_json(ques_obj, user_language):
    request_data = {'questions_id': str(ques_obj.id), 'question_type': ques_obj.question_type,
...
def answer_json(ans_obj, user_language):
    request_data = {'answer_id': ans_obj.answer_id, 'text': ans_obj.text[user_language],
                    'sub_questions': []}
    for sub_question in ans_obj.sub_questions:
        request_data['sub_questions'].append(sub_question_json(sub_question, user_language))
    return request_data


def sub_question_json(sub_ques_obj, user_language):
    request_data = {'sub_question_id': str(sub_ques_obj.sub_question_id), 'sub_question_type': sub_ques_obj.sub_question_type,
                    'text': sub_ques_obj.text[user_language], 'isMandatory': sub_ques_obj.isMandatory,
                    'sub_answers': []}
    for sub_answer in sub_ques_obj.sub_answers:
        request_data['sub_answers'].append(sub_answer_json(sub_answer, user_language))
    return request_data


def sub_answer_json(sub_ans_obj, user_language):
    request_data = {'sub_answer_id': str(sub_ans_obj.sub_answer_id), 'text': sub_ans_obj.text[user_language]}
    return request_data
